...
if(isset($_REQUEST['action']) && $_REQUEST['action'] == 'get') {
    ...
    $sql = "SELECT CONCAT(mf.name, ' ', m.name) AS name, son.model_id 
            FROM sold_out_notifications son LEFT JOIN model m ON son.model_id = m.modelid 
            LEFT JOIN manufacturer mf ON mf.manufacturerid = m.manufacturer_id 
            WHERE son.status = 0";
